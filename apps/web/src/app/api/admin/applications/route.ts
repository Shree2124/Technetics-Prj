  ...
  try {
    ...
    if (search) {
      ...
      applications = applications.filter((app: any) => {
        const citizenName = app.citizenId?.userId?.name?.toLowerCase() || "";
        const citizenEmail = app.citizenId?.userId?.email?.toLowerCase() || "";
        const schemeName = app.schemeId?.schemeName?.toLowerCase() || "";
        return (
          citizenName.includes(q) ||
          citizenEmail.includes(q) ||
